...
export const TableRangkuman = ({ data, onFetchData }) => {
        ...
        (item) =>
          ...
          (item.createdAt &&
            new Date(item.createdAt)
              ...
              .toLowerCase()
              .includes(searchValue.toLowerCase())) ||
          (item.performaBulanan &&
            item.performaBulanan
              .toLowerCase()
              .includes(searchValue.toLowerCase())) ||
          (item.actionPlan &&
            item.actionPlan.toLowerCase().includes(searchValue.toLowerCase()))
      );
    }
  ...
  const handleConfirmBulkDelete = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/laporan/rangkuman/bulk-delete", {
        method: "DELETE",
  ...
  const generatePDF = () => {
    try {
      ...
      doc.setFontSize(16);
      doc.text("Data Laporan Performa Bulanan", 14, 15);
      ...
      // Simpan dokumen
      const fileName = `Laporan_Performa_Bulanan_${now
        .toISOString()
        .slice(0, 10)}.pdf`;
      doc.save(fileName);
