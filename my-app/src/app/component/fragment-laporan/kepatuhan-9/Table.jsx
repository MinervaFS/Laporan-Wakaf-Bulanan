...
export const TableKepatuhan = ({ data, onFetchData }) => {
  ...
  const handleConfirmBulkDelete = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/laporan/kepatuhan/bulk-delete", {
        method: "DELETE",
  ...
  const generatePDF = () => {
    try {
      ...
      // Simpan dokumen
      const fileName = `Laporan_Kepatuhan-Hukum${now
        .toISOString()
        .slice(0, 10)}.pdf`;
      doc.save(fileName);
