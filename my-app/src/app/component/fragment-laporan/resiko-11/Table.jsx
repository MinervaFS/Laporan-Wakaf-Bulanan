...
export const TableResiko = ({ data, onFetchData }) => {
        ...
        (item) =>
          ...
          (item.createdAt &&
            new Date(item.createdAt)
              ...
              .includes(searchValue.toLowerCase())) ||
          (item.ResikoBaru &&
            item.ResikoBaru.toLowerCase().includes(
          ...
          (item.mitigasiResiko &&
            item.mitigasiResiko
              ...
              .includes(searchValue.toLowerCase())) ||
          (item.statusResiko &&
            item.statusResiko.toLowerCase().includes(searchValue.toLowerCase()))
      );
    }
  ...
  const handleConfirmBulkDelete = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/laporan/resiko/bulk-delete", {
        method: "DELETE",
  ...
  const generatePDF = () => {
    try {
      ...
      doc.setFontSize(16);
      doc.text("Data Laporan Mitigasi Resiko", 14, 15);
      ...
      autoTable(doc, {
        ...
        head: [
          [
            ...
            "Periode",
            "Tanggal",
            "Resiko Baru Yang Teridentifikasi",
            "Mitigasi Resiko",
            "Status Resiko",
            "Dibuat Oleh",
          ],
      ...
      // Simpan dokumen
      const fileName = `Laporan_Pengelolaan_Resiko_${now
        .toISOString()
        .slice(0, 10)}.pdf`;
      doc.save(fileName);
