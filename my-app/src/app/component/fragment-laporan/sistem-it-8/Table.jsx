...
export const TableSistemIt = ({ data, onFetchData }) => {
        ...
        (item) =>
          ...
          (item.createdAt &&
            new Date(item.createdAt)
              ...
              .toLowerCase()
              .includes(searchValue.toLowerCase())) ||
          (item.pengemabanganSistem &&
            item.pengemabanganSistem
              .toLowerCase()
              .includes(searchValue.toLowerCase())) ||
          (item.catatanTeknisNonTeknis &&
            item.catatanTeknisNonTeknis
              .toLowerCase()
              .includes(searchValue.toLowerCase()))
      );
  ...
  const handleConfirmBulkDelete = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/laporan/sistem-it/bulk-delete", {
        method: "DELETE",
  ...
  const generatePDF = () => {
    try {
      ...
      doc.setFontSize(16);
      doc.text("Data Laporan Sistem IT", 14, 15);
      ...
      // Simpan dokumen
      const fileName = `Laporan_Sistem-IT${now.toISOString().slice(0, 10)}.pdf`;
      doc.save(fileName);
