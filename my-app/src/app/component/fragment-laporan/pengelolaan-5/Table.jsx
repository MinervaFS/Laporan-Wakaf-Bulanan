...
export const TablePengelolaan = ({ data, onFetchData }) => {
  ...
  const handleConfirmBulkDelete = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/laporan/pengelolaan-asset/bulk-delete", {
        method: "DELETE",
    ...
    try {
      ...
      const tableBody = filteredData.map((item, index) => [
        ...
        item.createdAt
          ...
          : "-",
        item.jenisPengelolaanBaru || 0,
        item.penghasialAsset || 0,
        item.jenisPengelolaanBaru || "-",
        item.user?.role
          ? item.user.role
              .toLowerCase()
