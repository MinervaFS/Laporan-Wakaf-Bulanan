        ...
        (item) =>
          ...
          (item.createdAt &&
            new Date(item.createdAt)
              ...
              .toLowerCase()
              .includes(searchValue.toLowerCase())) ||
          (item.jumlahPelatihan &&
            item.jumlahPelatihan
              .toString()
              .toLowerCase()
              .includes(searchValue.toLowerCase())) ||
          (item.jumlahSdmDilatih &&
            item.jumlahSdmDilatih
              .toString()
              .toLowerCase()
              .includes(searchValue.toLowerCase())) ||
          (item.materi &&
            item.materi.toLowerCase().includes(searchValue.toLowerCase()))
      );
    }
    ...
    try {
      ...
      const tableBody = filteredData.map((item, index) => [
        ...
        item.createdAt
          ...
          : "-",
        item.jumlahPelatihan || 0,
        item.jumlahSdmDilatih || 0,
        item.materi || "-",
        item.user?.role
          ? item.user.role
              .toLowerCase()
      ...
      // Simpan dokumen
      const fileName = `Laporan_Kapasitas-SDM${now
        .toISOString()
        .slice(0, 10)}.pdf`;
      doc.save(fileName);
