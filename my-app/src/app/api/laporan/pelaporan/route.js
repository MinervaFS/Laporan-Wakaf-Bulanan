...
const pelaporantransaparasiSchema = z.object({
  ...
  jumlahLaporanPublik: z.coerce.number().min(1, "Jumlah Asset wajib diisi"),
...
export async function POST(req) {
  try {
    ...
    const createData = await prisma.pelaporantransaparasi.create({
      data: {
        name: parsed.name,
...
export async function DELETE(req) {
  try {
    ...
    const targetData = await prisma.pelaporantransaparasi.findUnique({
      where: {
        id: parseInt(id),
      },
      include: {
    ...
    if (role === "admin") {
      await prisma.pelaporantransaparasi.delete({
...
export async function GET() {
  try {
    ...
    let getpelaporantransaparasi;

    if (role === "admin") {
      getpelaporantransaparasi = await prisma.pelaporantransaparasi.findMany({
