...
const pengelolaanresikoSchema = z.object({
  name: z.string().min(1, "Nama wajib diisi"),
  periode: z.coerce.date({ required_error: "Periode wajib diisi" }),
  ResikoBaru: z.string().min(1, "Isi resiko teridentikasi"),
  mitigasiResiko: z.string().min(1, "Isi mitigasi resiko"),
  statusResiko: z.string().min(1, "Status Resiko"),
});
...
export async function POST(req) {
  try {
    ...
    const parsed = pengelolaanresikoSchema.parse(body);

    const createData = await prisma.pengelolaanresiko.create({
      data: {
        name: parsed.name,
        periode: parsed.periode,
        ResikoBaru: parsed.ResikoBaru,
        mitigasiResiko: parsed.mitigasiResiko,
        statusResiko: parsed.statusResiko,
...
export async function DELETE(req) {
  try {
    ...
    const targetData = await prisma.pengelolaanresiko.findUnique({
      where: {
        id: parseInt(id),
      },
      include: {
    ...
    if (role === "admin") {
      await prisma.pengelolaanresiko.delete({
...
export async function GET() {
  try {
    ...
    let getpengelolaanresiko;

    if (role === "admin") {
      getpengelolaanresiko = await prisma.pengelolaanresiko.findMany({
