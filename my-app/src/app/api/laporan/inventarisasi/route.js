...
const inventarisasiSchema = z.object({
  name: z.string().min(1, "Nama wajib diisi"),
  periode: z.coerce.date({ required_error: "Periode wajib diisi" }),
  jumlahAssetTerdaftar: z.coerce.number().min(1, "Jumlah Asset wajib diisi"),
  jumlahDokTerkait: z.coerce.number().min(1, "Jumlah Dokumen wajib diisi"),
  catatanInventarisasi: z.string().optional(),
});
...
export async function POST(req) {
  try {
    ...
    const parsed = inventarisasiSchema.parse(body);

    const createData = await prisma.inventarisasi.create({
      data: {
        name: parsed.name,
        periode: parsed.periode,
        jumlahAssetTerdaftar: parsed.jumlahAssetTerdaftar,
        jumlahDokTerkait: parsed.jumlahDokTerkait,
        catatanInventarisasi: parsed.catatanInventarisasi,
...
export async function DELETE(req) {
  try {
    ...
    const targetData = await prisma.inventarisasi.findUnique({
      where: {
        id: parseInt(id),
      },
      include: {
...
export async function GET() {
  try {
    ...
    let getinventarisasi;

    if (role === "admin") {
      getinventarisasi = await prisma.inventarisasi.findMany({
