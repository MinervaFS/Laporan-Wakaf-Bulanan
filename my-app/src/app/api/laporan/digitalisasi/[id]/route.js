...
const digitalisasidokSchema = z.object({
  ...
  dokumenTerdigitalisasi: z.coerce.number().optional(),
  totalDokdigitalisasi: z.coerce.number().optional(),
...
export async function PUT(req, { params }) {
  try {
    ...
    // Cari data existing
    const existing = await prisma.digitalisasidok.findUnique({
    ...
    // Update data
    const updated = await prisma.digitalisasidok.update({
      where: { id: parseInt(id) },
      data: {
        name: parsed.name,
