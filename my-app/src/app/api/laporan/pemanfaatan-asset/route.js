...
const pemanfaatanassetSchema = z.object({
  ...
  jumlahAssetDimanfaatkan: z.coerce
    .number()
    .min(1, "Jumlah Asset wajib dinilai"),
...
export async function POST(req) {
  try {
    ...
    const createData = await prisma.pemanfaatanasset.create({
      data: {
        name: parsed.name,
...
export async function DELETE(req) {
  try {
    ...
    const targetData = await prisma.pemanfaatanasset.findUnique({
      where: {
        id: parseInt(id),
      },
      include: {
    ...
    if (role === "admin") {
      await prisma.pemanfaatanasset.delete({
...
export async function GET() {
  try {
    ...
    let getpemanfaatanasset;

    if (role === "admin") {
      getpemanfaatanasset = await prisma.pemanfaatanasset.findMany({
