...
const rangkumanrekomendasiSchema = z.object({
  ...
  performaBulanan: z.string().min(1, "Isi performa bulanan"),
  actionPlan: z.string().min(1, "Isi action plan"),
});
...
export async function POST(req) {
  try {
    ...
    const createData = await prisma.rangkumanrekomendasi.create({
      data: {
        name: parsed.name,
...
export async function DELETE(req) {
  try {
    ...
    const targetData = await prisma.rangkumanrekomendasi.findUnique({
      where: {
        id: parseInt(id),
      },
      include: {
    ...
    if (role === "admin") {
      await prisma.rangkumanrekomendasi.delete({
...
export async function GET() {
  try {
    ...
    let getrangkumanrekomendasi;

    if (role === "admin") {
      getrangkumanrekomendasi = await prisma.rangkumanrekomendasi.findMany({
