...
const peningkatankapasitassdmSchema = z.object({
  ...
  jumlahPelatihan: z.coerce.number().min(1, "Jumlah Asset wajib diisi"),
  jumlahSdmDilatih: z.coerce.number().min(1, "Jumlah Dokumen wajib diisi"),
...
export async function POST(req) {
  try {
    ...
    const createData = await prisma.peningkatankapasitassdm.create({
      data: {
        name: parsed.name,
...
export async function DELETE(req) {
  try {
    ...
    const targetData = await prisma.peningkatankapasitassdm.findUnique({
      where: {
        id: parseInt(id),
      },
      include: {
    ...
    if (role === "admin") {
      await prisma.peningkatankapasitassdm.delete({
...
export async function GET() {
  try {
    ...
    let getpeningkatankapasitassdm;

    if (role === "admin") {
      getpeningkatankapasitassdm =
        await prisma.peningkatankapasitassdm.findMany({
    ...
    return NextResponse.json({
      ...
      data: getpeningkatankapasitassdm,
    });
  } catch (error) {
    return NextResponse.json(
      { message: "Terjadi kesalahan server", error: error.message },
