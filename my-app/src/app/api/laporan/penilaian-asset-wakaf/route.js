...
const penilaianassetwakafSchema = z.object({
  name: z.string().min(1, "Nama wajib diisi"),
  periode: z.coerce.date({ required_error: "Periode wajib diisi" }),
  jumlahAssetDinilai: z.coerce.number().min(1, "Jumlah Asset wajib dinilai"),
  nilaiAssetWakaf: z.coerce.number().min(1, "nilai asset wajib currency"),
  catatanPenilaianAsset: z.string().optional(),
});
...
export async function POST(req) {
  try {
    ...
    const parsed = penilaianassetwakafSchema.parse(body);

    const createData = await prisma.penilaianassetwakaf.create({
      data: {
        name: parsed.name,
        periode: parsed.periode,
        jumlahAssetDinilai: parsed.jumlahAssetDinilai,
        nilaiAssetWakaf: parsed.nilaiAssetWakaf,
        catatanPenilaianAsset: parsed.catatanPenilaianAsset,
...
export async function DELETE(req) {
  try {
    ...
    const targetData = await prisma.penilaianassetwakaf.findUnique({
      where: {
        id: parseInt(id),
      },
      include: {
    ...
    if (role === "admin") {
      await prisma.penilaianassetwakaf.delete({
...
export async function GET() {
  try {
    ...
    let getpenilaianassetwakaf;

    if (role === "admin") {
      getpenilaianassetwakaf = await prisma.penilaianassetwakaf.findMany({
